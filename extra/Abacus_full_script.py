...
d1, d2, dG2, d3 = generate_fields_new_growth(dlin, prefactor, zic, zout, comm=comm)
...
plt.xlabel("$y\,[h^{-1}\,\\mathrm{Mpc}]$")
plt.ylabel("$z\,[h^{-1}\,\\mathrm{Mpc}]$")
plt.savefig(output_folder + 'Abacus_truth_bestfitcubic_diff_z=%.1f_yz_Nmesh_%i.pdf'%(zout, Nmesh), bbox_inches='tight')
plt.close()
...
path_to_save = output_folder + "Abacus_zout_%.1f_grid_Nmesh_%i"%(zout, Nmesh)
...
np.save(output_folder+"Abacus_best_fit_field_zout_%.1f_grid_Nmesh_%i"%(zout, Nmesh), final_field.value)
...
plt.savefig(output_folder + 'Abacus_betas_z=%.1f_yz_Nmesh_%i.pdf'%(zout, Nmesh), bbox_inches='tight')
plt.close()
...
plt.savefig(output_folder + 'Abacus_Pk_z=%.1f_yz_Nmesh_%i.pdf'%(zout, Nmesh), bbox_inches='tight')
plt.close()
